...
LOG = open(os.path.realpath(__file__)+"/../"+config['server']['log_file'], "a")
...
def send_file_directory(client_socket):
    try:
        list_of_dir = os.listdir(os.path.realpath(__file__)+'\..\\'+WATCH_FOLDER_NAME+'\\')
...
def send_files(client_socket, files):
    try:
        fds = [open(os.path.realpath(__file__)+"\..\\"+WATCH_FOLDER_NAME+"\\"+files[i],'r') for i in range(len(files))]
